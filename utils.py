...
#weight1 query对应的权重，weight2 emb对应的权重
def get_nn_avg_dist(emb, query, knn):
    ...
    if FAISS_AVAILABLE:
        ...
        for i in range(0, query.shape[0], bs): #要查询的词向量的数量
...
def read_txt_embeddings(params, source, full_vocab):
    ...
    weight_path = params.src_weight if source else params.tgt_weight
    if os.path.isfile(weight_path):
        with io.open(weight_path, 'r', encoding='utf-8', newline='\n', errors='ignore') as f:
            for i, line in enumerate(f):
                word, wt = line.rstrip().split(' ', 1)
                if word in word2id:
                    weights[word2id[word]] = float(wt)

    assert embeddings.size() == (len(dico), params.emb_dim)
    assert len(embeddings) == len(weights)
    return dico, embeddings, weights
...
#full_vocab仅考虑重复项目
def load_embeddings(params, source, full_vocab=False, full_embedding=False):
    """
    ...
    - 'full_embedding not equal embedding.full embedding for exporting but origianl embedding for train which is 
    much less than full embedding.'
    """
    assert type(source) is bool and type(full_vocab) is bool
    emb_path = ''
    if full_embedding:
        emb_path = params.full_src_emb if source else params.full_tgt_emb
    else:
        emb_path = params.src_emb if source else params.tgt_emb
